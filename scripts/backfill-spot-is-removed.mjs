...
const shouldWrite = process.argv.includes('--yes-backfill-is-removed');
...
async function runWithFirebaseCliAuth() {
  ...
  do {
    ...
    const result = await requestFirestore(
      accessToken,
      `projects/${PROJECT_ID}/databases/(default)/documents/spots?${params.toString()}`
    );
  ...
  const docsMissingIsRemoved = documents.filter((document) => !document.fields?.isRemoved);

  console.log(`Scanned ${documents.length} spot document(s).`);
  console.log(`Will update ${docsMissingIsRemoved.length} spot document(s) missing isRemoved.`);
  console.log('Documents with isRemoved: true will not be changed.');

  if (!shouldWrite) {
    console.log('Dry run only. Re-run with --yes-backfill-is-removed to apply the backfill.');
    return;
  }

  let updatedCount = 0;

  for (let index = 0; index < docsMissingIsRemoved.length; index += BATCH_LIMIT) {
    const chunk = docsMissingIsRemoved.slice(index, index + BATCH_LIMIT);
        ...
        body: JSON.stringify({
          writes: chunk.map((document) => ({
            update: {
              name: document.name,
              fields: {
                isRemoved: { booleanValue: false },
              },
            },
            updateMask: {
              fieldPaths: ['isRemoved'],
            },
            updateTransforms: [
              {
                fieldPath: 'updatedAt',
  ...
  console.log(`Backfill complete. Updated ${updatedCount} spot document(s).`);
}
...
async function runWithServiceAccount() {
  ...
  const docsMissingIsRemoved = snapshot.docs.filter((spotDoc) => !Object.hasOwn(spotDoc.data(), 'isRemoved'));

  console.log(`Scanned ${snapshot.size} spot document(s).`);
  console.log(`Will update ${docsMissingIsRemoved.length} spot document(s) missing isRemoved.`);
  console.log('Documents with isRemoved: true will not be changed.');
  ...
  for (const spotDoc of docsMissingIsRemoved) {
...
main().catch((error) => {
  console.error('[Backfill] Failed to backfill spots.isRemoved', error);
  process.exitCode = 1;
});
